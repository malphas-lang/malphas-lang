...
func (c *Checker) processModDecl(modDecl *ast.ModDecl, currentFile *ast.File) {
	...
	if c.LoadingModules[moduleName] {
		// Find the chain of modules that lead to this circular dependency
		chain := []string{moduleName}
...
func (c *Checker) processUseDecl(useDecl *ast.UseDecl) {
	if len(useDecl.Path) == 0 {
		c.reportModuleError(
			"use path cannot be empty",
			diag.CodeTypeInvalidOperation,
			useDecl.Span(),
			"a `use` declaration must specify a path to import\n  example: use std::collections::HashMap;",
			lexer.Span{},
		)
		return
	}
...
func (c *Checker) resolveModulePath(path []string, span lexer.Span) Type {
	if len(path) == 0 {
		c.reportModuleError(
			...
			diag.CodeTypeInvalidOperation,
			span,
			"a module path must contain at least one component\n  example: use std::collections;",
			lexer.Span{},
		)
		return nil
	}
	...
	if path[0] == "std" {
		...
		msg := fmt.Sprintf("could not resolve standard library path `%s`", pathStr)
	...
	similarMod := ""
	for modName := range c.Modules {
		if editDistance(path[0], modName) <= 2 {
			similarMod = modName
			break
		}
	}
	...
	if similarMod != "" {
		help = fmt.Sprintf("did you mean `%s`?\n"+
			"ensure the module is declared with `mod %s;` before use", similarMod, similarMod)
	} else {
